...
export const devices = sqliteTable('devices', { id: integer().primaryKey({ autoIncrement: true }), userId: text('user_id').notNull().references(() => users.id), deviceId: text('device_id'), platform: text(), model: text(), userAgent: text('user_agent').notNull(), pushToken: text('push_token'), resetCount: integer('reset_count').notNull().default(0), createdAt: integer('created_at').notNull(), updatedAt: integer('updated_at').notNull(), lastSeenAt: integer('last_seen_at').notNull() });
