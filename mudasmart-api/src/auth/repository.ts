...
const userFields = { id: users.id, email: users.email, fullName: users.fullName, role: users.role, isActive: users.isActive };
...
export const repository = {
  ...
  userByEmail: db.select({ ...userFields, passwordHash: users.passwordHash }).from(users).where(eq(users.email, p('email'))).prepare(),
  userById: db.select(userFields).from(users).where(eq(users.id, p('id'))).prepare(),
  activeUserById: db.select(userFields).from(users).where(and(eq(users.id, p('id')), eq(users.isActive, true))).prepare(),
  ...
  touchDevice: db.update(devices).set({ platform: p('platform'), model: p('model'), userAgent: p('userAgent'), updatedAt: p('now'), lastSeenAt: p('now') }).where(eq(devices.id, p('id'))).prepare(),
...
export type User = { id: string; email: string; fullName: string; role: Role; isActive: boolean };
